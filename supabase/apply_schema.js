...
// apply_schema.js
// Applies the 01_init_schema.sql DDL to the remote Supabase project.
...
const sqlPath = path.join(__dirname, 'migrations', '01_init_schema.sql');
...
const tempJsonPath = path.join(__dirname, 'temp_payload.json');
fs.writeFileSync(tempJsonPath, JSON.stringify(payload));

try {
  console.log(`Applying migration schema to Supabase project ${projectRef}...`);
  
  // Use curl with file payload to bypass undici TLS/certificate issues and handle huge queries safely
  ...
  if (httpStatus === '200' || httpStatus === '201') {
    console.log('Success! Database schema successfully updated.');
    console.log(body);
  } else {
    console.error('Error applying migration schema:');
    console.error(body);
    process.exit(1);
  }
} catch (error) {
